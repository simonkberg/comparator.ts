...
| [localeComparator](Function.localeComparator.md) | Creates a [Comparator](Interface.Comparator.md) for comparing strings using locale-aware ordering. The result is equivalent to `String.prototype.localeCompare` called with the same `locales` and `options`. |
